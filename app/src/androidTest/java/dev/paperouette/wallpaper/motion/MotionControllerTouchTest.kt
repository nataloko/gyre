...
class MotionControllerTouchTest {
    ...
    fun nudgeSpinsOnASingleTapOnlyWhenItIsTurnedOn() {
        ...
        val settings = PaperouetteSettings(tapToSpin = true)
    ...
    fun heldStillNothingMoves() {
        val settings = PaperouetteSettings(
            stillArtwork = true,
            tapToSpin = true,
            tiltEnabled = true,
            flickEnabled = true,
    ...
    fun comingOffHoldStillDoesNotResumeMidFlight() {
        val moving = PaperouetteSettings(tapToSpin = true)
        down(CENTRE_X, CENTRE_Y, moving)
        up(CENTRE_X, CENTRE_Y, moving)
        controller.snapshot(0L)

        controller.updateSettings(PaperouetteSettings(stillArtwork = true, tapToSpin = true))
        controller.snapshot(HALF_SECOND)
    ...
    fun aNudgeFollowsReverse() {
        fun nudge(reversed: Boolean): Float {
            ...
            controller = MotionController(context)
            val settings = PaperouetteSettings(tapToSpin = true, rotationReversed = reversed)
            down(CENTRE_X, CENTRE_Y, settings)
            up(CENTRE_X, CENTRE_Y, settings)
            controller.snapshot(HALF_SECOND)
    ...
    fun theLauncherZoomIsHeldUntilTheNextFrame() {
        controller.updateSettings(PaperouetteSettings(launcherZoomEnabled = true))
    ...
    fun turningTheLauncherZoomOffLetsTheArtworkBack() {
        ...
        controller.updateSettings(PaperouetteSettings(launcherZoomEnabled = false))
    ...
    private fun dispatch(event: MotionEvent, settings: PaperouetteSettings) {
        try {
    ...
    private fun down(x: Float, y: Float, settings: PaperouetteSettings = PaperouetteSettings()) =
        dispatch(single(MotionEvent.ACTION_DOWN, x, y), settings)

    private fun move(x: Float, y: Float, settings: PaperouetteSettings = PaperouetteSettings()) =
        dispatch(single(MotionEvent.ACTION_MOVE, x, y), settings)

    private fun up(x: Float, y: Float, settings: PaperouetteSettings = PaperouetteSettings()) =
        dispatch(single(MotionEvent.ACTION_UP, x, y), settings)

    private fun cancel(x: Float, y: Float, settings: PaperouetteSettings = PaperouetteSettings()) =
    ...
    private fun pointerUp(points: List<Pair<Float, Float>>, index: Int) =
        dispatch(multi(MotionEvent.ACTION_POINTER_UP, points, index), PaperouetteSettings())

    private fun multiMove(points: List<Pair<Float, Float>>) =
        dispatch(multi(MotionEvent.ACTION_MOVE, points, 0), PaperouetteSettings())
