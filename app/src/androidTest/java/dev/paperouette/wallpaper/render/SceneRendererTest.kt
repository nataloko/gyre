...
import dev.paperouette.wallpaper.data.ArtworkPaths
import dev.paperouette.wallpaper.data.BundledCatalogRepository
import dev.paperouette.wallpaper.data.openStream
...
class SceneRendererTest {
    ...
    fun representativeZeroMotionScenesMatchBundledPreviews() {
        ...
        OffscreenRenderer(context).use { renderer ->
            cases.forEach { limit ->
                ...
                Log.i("PaperouetteVisual", "${remix.id}: meanRgb=$error ssim=$ssim")
    ...
    fun mirroringRendersTheReflectionOfTheSameFrame() {
        ...
        OffscreenRenderer(context).use { renderer ->
            ...
            Log.i("PaperouetteVisual", "mirror versus flip error=$error")
    ...
    fun aMirroredSceneStillKeepsItsEdgesOutOfView() {
        ...
        OffscreenRenderer(context).use { renderer ->
            for (step in 0 until 5) {
                ...
                Log.i("PaperouetteVisual", "mirrored step $step cleared=$cleared")
                assertTrue(
    ...
    fun reversingRunsTheSameAnimationBackwards() {
        ...
        OffscreenRenderer(context).use { renderer ->
            ...
            Log.i("PaperouetteVisual", "reversed versus rewound error=$error")
    ...
    fun reverseTurnsTheArtworkTheOtherWay() {
        ...
        OffscreenRenderer(context).use { renderer ->
            ...
            Log.i("PaperouetteVisual", "travel forward=$forward reversed=$reversed")
    ...
    fun anOffCentreRotationCentreStillFillsTheScreen() {
        ...
        OffscreenRenderer(context).use { renderer ->
            ...
            for (step in 0 until 5) {
                ...
                val cleared = frame.fractionMatching(CLEAR_COLOR)
                Log.i("PaperouetteVisual", "offCentre step $step cleared=$cleared")
                assertTrue(
                    "background showed through on ${(cleared * 100).toInt()}% of the frame",
                    cleared < MAX_CLEARED_FRACTION,
                )
    ...
    fun aLandscapeOffCentreRotationCentreStillFillsTheScreen() {
        ...
        OffscreenRenderer(context, width = 320, height = 180).use { renderer ->
            ...
            for (step in 0 until 5) {
                ...
                val cleared = frame.fractionMatching(CLEAR_COLOR)
                Log.i("PaperouetteVisual", "landscape offCentre step $step cleared=$cleared")
                assertTrue(
                    "background showed through on ${(cleared * 100).toInt()}% of the frame",
                    cleared < MAX_CLEARED_FRACTION,
                )
