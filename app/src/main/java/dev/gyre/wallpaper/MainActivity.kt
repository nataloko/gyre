...
import dev.gyre.wallpaper.data.DocumentTreeImportSource
import dev.gyre.wallpaper.data.ImportSource
import dev.gyre.wallpaper.data.ZipImportSource
import dev.gyre.wallpaper.ui.GyreApp
import dev.gyre.wallpaper.wallpaper.GyreWallpaperService
...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val application = application as GyreApplication
        setContent {
            GyreApp(
    ...
    private fun openWallpaperPreview(): Boolean {
        val component = ComponentName(this, GyreWallpaperService::class.java)
    ...
    /**
     * The system's own file picker, which is the only place Gyre asks for anything outside itself.
     *
    ...
    private fun import(uri: Uri, source: (release: () -> Unit) -> ImportSource) {
        ...
        (application as GyreApplication).importer.start {
            source {
                if (persisted) {
                    runCatching {
