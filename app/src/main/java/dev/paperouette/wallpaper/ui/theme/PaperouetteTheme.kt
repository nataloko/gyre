...
@Composable
fun PaperouetteTheme(palette: PaletteColors? = null, content: @Composable () -> Unit) {
    val target = remember(palette) {
        ArtworkColorScheme.chassis(palette ?: FALLBACK_PALETTE)
    }
    MaterialTheme(
        colorScheme = target.animated(),
        shapes = PaperouetteShapes,
        typography = PaperouetteTypography,
        content = content,
    )
}
...
private fun ColorScheme.animated(): ColorScheme {
    ...
    fun Color.eased(label: String) =
        animateColorAsState(this, PaperouetteMotion.colorWash(), label = label).value
