...
fun PaperouetteApp(
    ...
    val previewView = remember(context) { PaperouettePreviewView(context) }
    DisposableEffect(previewView) { onDispose(previewView::release) }

    PaperouetteTheme(palette = activeRemix.colors) {
...
private fun PlayHint(playing: Boolean, modifier: Modifier) {
    ...
    AnimatedVisibility(
        ...
        enter = fadeIn(PaperouetteMotion.effects()),
        exit = fadeOut(PaperouetteMotion.effects()),
    ) {
        Text(
...
private fun OverlayTransition(visible: Boolean, content: @Composable () -> Unit) {
    AnimatedVisibility(
        visible = visible,
        enter = fadeIn(PaperouetteMotion.effects()) +
            slideInVertically(PaperouetteMotion.spatial()) { height -> height / 10 },
        exit = fadeOut(PaperouetteMotion.effects()) +
            slideOutVertically(PaperouetteMotion.spatial()) { height -> height / 10 },
    ) {
        content()
    }
}
