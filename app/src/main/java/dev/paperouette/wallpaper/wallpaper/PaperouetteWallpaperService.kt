...
class PaperouetteWallpaperService : WallpaperService() {
    private val engines = mutableSetOf<PaperouetteEngine>()

    /** Shared across engines: the measurement is the same for every one of them. */
    private val tone by lazy { SceneTone(this) }

    override fun onCreateEngine(): Engine = PaperouetteEngine().also(engines::add)

    override fun onConfigurationChanged(newConfig: Configuration) {
        super.onConfigurationChanged(newConfig)
        engines.toList().forEach(PaperouetteEngine::onSystemAppearanceChanged)
    }

    override fun onTrimMemory(level: Int) {
        super.onTrimMemory(level)
        engines.toList().forEach(PaperouetteEngine::trimMemory)
    }

    private inner class PaperouetteEngine : Engine() {
        private val application = this@PaperouetteWallpaperService.application as PaperouetteApplication
        ...
        private val motion = MotionController(this@PaperouetteWallpaperService)
        private val renderHost = EglRenderHost(
            context = this@PaperouetteWallpaperService,
